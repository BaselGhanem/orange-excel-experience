...
const DEFAULT_CONFIG={siteLive:true,announcementEnabled:false,announcementText:``,assessmentEnabled:true,assessmentPhase:`pre`,attendanceEnabled:false,attendanceDay:0,dayVisibility:{day1:false,day2:false,day3:false},files:DEFAULT_FILES};
...
const renderSite=()=>{if($(`announcementText`))$(`announcementText`).value=config.announcementText||``;if($(`announcementEnabled`))$(`announcementEnabled`).checked=!!config.announcementEnabled};
...
const renderAll=()=>{renderStatus();renderDays();renderAssessment();renderAttendanceMode();renderSite()};
const showAdmin=async()=>{$(`loginView`).classList.add(`hidden`);$(`adminView`).classList.remove(`hidden`);await loadConfig();renderAll();await Promise.all([loadResults(),loadAttendance()]);lucide.createIcons()};
