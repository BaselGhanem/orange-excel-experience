(async()=>{
  ...
  const STATIC_IDS={1:[`d1f1`,`d1f2`,`d1f3`,`d1f4`],2:[`d2f1`,`d2f2`,`d2f3`,`d2f4`,`d2f5`],3:[`d3f1`,`d3f2`,`d3f3`]};
  const syncCourseFiles=(raw)=>{if(!pageDay)return;let count=0;(STATIC_IDS[pageDay]||[]).forEach((id)=>{const el=document.getElementById(id);if(!el)return;const visible=!!raw.files?.[id];el.classList.toggle(`show`,visible);if(visible)count+=1});document.querySelectorAll(`[data-live-custom-file="1"]`).forEach((el)=>el.remove());const box=document.querySelector(`.files`);(Array.isArray(raw.customFiles)?raw.customFiles:[]).filter((f)=>Number(f.day)===pageDay&&f.enabled).forEach((f)=>{if(!f.id||!f.fileName||/[\\/]/.test(f.fileName)||f.fileName.includes(`..`))return;const el=document.createElement(`div`);el.id=f.id;el.dataset.liveCustomFile=`1`;el.className=`file show`;el.innerHTML=`<div class="icon"><i data-lucide="file-spreadsheet"></i></div><div><b>${esc(f.title||f.fileName)}</b><small>${esc(f.sub||`Additional Material`)}</small></div><a class="download" href="../downloads/day${pageDay}/${encodeURIComponent(f.fileName)}" download><i data-lucide="download" size="17"></i></a>`;box?.appendChild(el);count+=1});const empty=document.getElementById(`empty`);if(empty)empty.style.display=count?`none`:`block`;if(window.lucide)lucide.createIcons()};
  let configuredDay=1;
